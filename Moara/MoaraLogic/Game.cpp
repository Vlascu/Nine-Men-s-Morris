...
IGamePtr IGame::Produce(GameConfigPtr& gameConfig)
{
	...
	{
		LOG.error("Failed to produce Game");
		return {};
	}
	return game;
}
...
void Game::LoadGame(const std::string& filePath)
{
	...
	if (!file.is_open())
	{
		throw FileException("Can't load the file.");
	}
	...
	LOG.info("Loaded the game from {%s}", filePath);
...
void Game::SaveGame(const std::string& filePath) const
{
	...
	LOG.info("Saved the game to {%s}", filePath);
