...
TEST_F(GameTest, WinningPlayer)
{
	PlayerConfig fourPlayersConfig;
	fourPlayersConfig.SetPlayersConfig(std::make_pair(0, 0), std::make_pair(1, 0), std::make_pair(2, 0), std::make_pair(3, 0));

	m_game->SetPlayerConfig(fourPlayersConfig);
	m_game->LoadGame("../MoaraTests/Winning_Test_Initialization.txt");

	EXPECT_EQ(m_game->GetWinningPlayer(), EPieceType::None);
	EXPECT_NE(m_game->GetGameState(), EGameState::Finished);
	EXPECT_EQ(m_game->GetAllPlayerTypes().size(), 4);

	m_game->MovePiece(2, 6);

	EXPECT_EQ(m_game->GetWinningPlayer(), EPieceType::None);
	EXPECT_NE(m_game->GetGameState(), EGameState::Finished);
	EXPECT_EQ(m_game->GetAllPlayerTypes().size(), 3);

	m_game->MovePiece(10, 9);
	m_game->RemovePiece(12);

	EXPECT_EQ(m_game->GetWinningPlayer(), EPieceType::None);
	EXPECT_NE(m_game->GetGameState(), EGameState::Finished);
	EXPECT_EQ(m_game->GetAllPlayerTypes().size(), 2);

	m_game->MovePiece(3, 2);
	m_game->RemovePiece(7);

	EXPECT_EQ(m_game->GetWinningPlayer(), EPieceType::Black);
	EXPECT_EQ(m_game->GetGameState(), EGameState::Finished);
	EXPECT_EQ(m_game->GetAllPlayerTypes().size(), 1);
}
...
TEST_F(GameTest, UndoTests)
{
	//not mocked board
	m_game->LoadGame("../MoaraTests/Undo_Test_Load.txt");

	std::vector<GameStatus> gameStats = {
		{6, 7, EPlayerType::White, EGameState::Moving}
		, {6, 7, EPlayerType::Black, EGameState::Moving}
		, {7, 7, EPlayerType::White, EGameState::Removing}
		, {7, 7, EPlayerType::White, EGameState::Moving}
		, {7, 7, EPlayerType::Black, EGameState::Moving}
		, {7, 6, EPlayerType::White, EGameState::Placing}
		, {6, 6, EPlayerType::Black, EGameState::Placing}
		, {7, 6, EPlayerType::White, EGameState::Removing}
		, {7, 5, EPlayerType::White, EGameState::Placing}
		, {6, 5, EPlayerType::Black, EGameState::Placing}
		, {6, 4, EPlayerType::White, EGameState::Placing}
		, {5, 4, EPlayerType::Black, EGameState::Placing}
		, {5, 3, EPlayerType::White, EGameState::Placing}
		, {5, 4, EPlayerType::Black, EGameState::Removing}
		, {4, 4, EPlayerType::Black, EGameState::Placing}, };

	for (const auto& gameStatus : gameStats)
	{
		EXPECT_EQ(m_game->GetPlayerPiecesOnBoard(EPieceType::Black), gameStatus.player1Pieces);
		EXPECT_EQ(m_game->GetPlayerPiecesOnBoard(EPieceType::White), gameStatus.player2Pieces);
		EXPECT_EQ(m_game->GetActivePlayer(), gameStatus.activePlayer);
		EXPECT_EQ(m_game->GetGameState(), gameStatus.gameState);

		m_game->Undo();
	}

	m_game->SaveGame("../MoaraTests/Undo_Test_Save.txt");
}
...
TEST_F(GameTest, EasyComputerRemoving)
{
	RemoveTest(m_easyComputer);
}

TEST_F(GameTest, MediumComputerPlacingFullLine)
{
	PlacingTest(m_mediumComputer, "../MoaraTests/medium_computer_place.txt", 1, EPieceType::Black);
}

TEST_F(GameTest, MediumComputerPlacingBlocking)
{
	PlacingTest(m_mediumComputer, "../MoaraTests/medium_computer_place.txt", 1, EPieceType::White);
}

TEST_F(GameTest, MediumComputerRemoving)
{
	RemoveTest(m_mediumComputer);
}

TEST_F(GameTest, MediumComputerMovingFullLine)
{
	MovingTest(m_mediumComputer, "../MoaraTests/medium_computer_moving.txt", MovePosition(6, 4), EPieceType::Black);
}

TEST_F(GameTest, MediumComputerMovingBlocking)
{
	MovingTest(m_mediumComputer, "../MoaraTests/medium_computer_moving.txt", MovePosition(3, 1), EPieceType::White);
}

TEST_F(GameTest, HardComputerPlacingFullLine)
{
	PlacingTest(m_hardComputer, "../MoaraTests/medium_computer_place.txt", 1, EPieceType::Black);
}

TEST_F(GameTest, HardComputerPlacingBlocking)
{
	PlacingTest(m_hardComputer, "../MoaraTests/medium_computer_place.txt", 1, EPieceType::White);
}

TEST_F(GameTest, HardComputerMovingFullLine)
{
	MovingTest(m_hardComputer, "../MoaraTests/medium_computer_moving.txt", MovePosition(6, 4), EPieceType::Black);
}

TEST_F(GameTest, HardComputerMovingBlocking)
{
	MovingTest(m_hardComputer, "../MoaraTests/medium_computer_moving.txt", MovePosition(3, 1), EPieceType::White);
}

TEST_F(GameTest, HardComputerAdvancedFullLine)
{
	MovingTest(m_hardComputer, "../MoaraTests/hard_computer_advanced_full.txt", MovePosition(4, 3), EPieceType::Black);
}

TEST_F(GameTest, HardComputerAdvancedBlock)
{
	MovingTest(m_hardComputer, "../MoaraTests/hard_computer_advanced_block.txt", MovePosition(4, 3), EPieceType::White);
}

TEST_F(GameTest, HardComputerRemoving)
{
	RemoveTest(m_hardComputer);
}

TEST_F(GameTest, AddRemoveListener)
{
	EXPECT_EQ(m_game->HowManyListeners(), 0);

	m_game->AddListener(std::make_shared<ListenerDummy>());

	EXPECT_EQ(m_game->HowManyListeners(), 1);

	m_game->RemoveAllListeners();

	EXPECT_EQ(m_game->HowManyListeners(), 0);

	std::shared_ptr<IGameListener> listener = std::make_shared<ListenerDummy>();

	m_game->AddListener(std::make_shared<ListenerDummy>());
	m_game->AddListener(listener);

	m_game->RemoveListener(listener);
